...
local=Path(sys.executable).parent/('cmake.exe' if os.name=='nt' else 'cmake')
cmake=str(bundled) if bundled.is_file() else str(local) if local.is_file() else shutil.which('cmake')
