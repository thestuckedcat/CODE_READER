...
selectFunction('alias_write');
assert.ok(document.getElementById('detail').textContent.includes('字段访问'));
assert.ok(document.getElementById('aliases').textContent.includes('shared_pair.left'));
assert.ok(document.getElementById('aliases').textContent.includes('回调候选'));
selectFunction('multi_nested_write');
assert.ok(document.getElementById('aliases').textContent.includes('nested_a.pair.left'));
assert.ok(document.getElementById('aliases').textContent.includes('nested_b.pair.left'));
selectFunction('callback_lifecycle');
assert.ok(document.getElementById('aliases').textContent.includes('cleared'));
selectFunction('wrapper_locked_write');
assert.ok(document.getElementById('detail').textContent.includes('锁事件 2'));
assert.ok(document.getElementById('locks').textContent.includes('shared_counter'));
selectFunction('dispatch');
for(const option of document.querySelectorAll('#direction option'))option.selected=option.value==='down';
document.getElementById('direction').onchange();
const virtualDetail=document.querySelector('#tree details');virtualDetail.open=true;virtualDetail.dispatchEvent(new window.Event('toggle'));
assert.ok(document.getElementById('tree').textContent.includes('open_world'));
assert.ok(document.getElementById('tree').textContent.includes('DerivedA'));
assert.ok(document.getElementById('tree').textContent.includes('DerivedB'));
console.log('PASS: viewer search, paths, boundaries, objects, locks, shared-state, nested fields, callback states and virtual candidates (DOM; not browser layout).');
