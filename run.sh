...
ATLAS_PY="$ATLAS_DIR/runtime/$ATLAS_PLATFORM/python/bin/python3.12"
if [ ! -f "$ATLAS_PY" ]; then echo 'Bundled Python missing; use the full distribution ZIP.' >&2; exit 2; fi
# ZIP extractors may discard Unix executable bits.
chmod u+x "$ATLAS_PY" "$ATLAS_DIR/runtime/$ATLAS_PLATFORM/site/cmake/data/bin/cmake" "$ATLAS_DIR/runtime/$ATLAS_PLATFORM/site/ninja/data/bin/ninja" 2>/dev/null || true
