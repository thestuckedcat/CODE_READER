...
`alias_analysis.json` 与相邻 JSONL 产物记录有限别名闭包、多目标对象集合、嵌套字段路径、跨调用实参/形参对象、锁所有权摘要、函数指针字段事件及同函数源码序状态。`callback_states.jsonl` 单列最终 configured/cleared/may_multiple 状态。数组、强制转换、指针算术和多目标情况不会被提升为唯一精确关系。
